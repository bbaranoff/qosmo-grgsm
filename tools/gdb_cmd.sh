#!/bin/bash
# =============================================================================
#  gdb_cmd.sh — genere cmd.gdb : le panneau de commandes gdb du banc osmocom
# =============================================================================
#  ./gdb_cmd.sh              ecrit tools/cmd.gdb (a cote de ce script)
#  ./gdb_cmd.sh --install    idem + ajoute `source .../cmd.gdb` a ~/.gdbinit
#  ./gdb_cmd.sh --out FICH   ecrit ailleurs
#
#  cmd.gdb est source AUTOMATIQUEMENT par la console telnet (tools/gdb-telnet.py,
#  `telnet localhost 44444`) et par gdb -x. Dedans : `help_osmo` liste tout.
#  Les commandes lisent le firmware osmocom-bb PAR SES SYMBOLES (l1s, dsp_api,
#  fbs, rxnb, afc_state...) : il faut l'ELF non strippe (layer1.highram.elf),
#  c'est celui que -kernel charge et que le lanceur passe a gdb.
# -----------------------------------------------------------------------------
set -eu
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
OUT="$HERE/cmd.gdb"; INSTALL=0
while [ $# -gt 0 ]; do
    case "$1" in
        --install) INSTALL=1 ;;
        --out) OUT="$2"; shift ;;
...
echo "ecrit : $OUT ($(wc -l < "$OUT") lignes, $(grep -c '^define ' "$OUT") commandes)"
if [ "$INSTALL" = 1 ]; then
    touch ~/.gdbinit
    grep -qF "source $OUT" ~/.gdbinit || printf 'source %s\n' "$OUT" >> ~/.gdbinit
    echo "~/.gdbinit : source $OUT"
fi
